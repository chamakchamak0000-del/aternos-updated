      ...
      <body>
        <div class="container" id="main-container">
          ...
          <div class="stat-card">
            ...
            <div class="value" id="active-bot-text">Waiting...</div>
          </div>
        ...
        <script>
          ...
          const updateStats = async () => {
            try {
              ...
              // Update Active Bot
              const activeBotText = document.getElementById('active-bot-text');
              activeBotText.innerText = data.activeBot
                ? \`\${data.activeBot}\${data.pendingBot ? ' (switching to ' + data.pendingBot + ')' : ''}\`
                : 'None';
...
app.get('/health', (req, res) => {
  res.json({
    ...
    activeBot: botState.activeBot,
    pendingBot: manager.getStats().pendingBot,
    poolSize: manager.getStats().poolSize
  });
});
...
// ============================================================
// BOT ROTATION — 14-account handover via BotManager
//
// Fixes applied to the old single-bot reconnect system:
//   - The old scheduleReconnect() could clear its own pending
//     reconnect timeout while bailing out early (because 'kicked'
//     and 'end' both fired for the same disconnect), leaving
//     isReconnecting stuck "true" forever with no timer left to
//     ever flip it back — the bot would silently never reconnect.
//   - The 60s "no spawn" connectionTimeout was never cleared on
//     'end'/'kicked'/error, so it could fire later against a
//     totally different (already-connected) bot instance.
//   - leaveRejoin.js used to force the single bot to quit on its
//     own random 1-5 minute timer — that directly violates "never
//     let the old bot leave before its replacement has joined", so
//     that self-quit timer has been removed (see leaveRejoin.js).
// All of that is replaced by BotManager, which only ever tracks one
// active bot + at most one in-flight replacement/candidate, guarded
// by attempt IDs so stale events from superseded bots are ignored.
// ============================================================
let bot = null; // always mirrors the current manager.active bot, for the dashboard/console below
...
function createIntervalManager() {
  ...
  return {
    ...
    clearAll() {
      console.log(`[Cleanup] Clearing ${ids.length} intervals`);
...
// Intervals belong to whichever bot is currently active — a fresh
// manager is created per activation so a handover can never clear
// (or leak) the wrong bot's timers.
let currentIntervals = createIntervalManager();

const manager = new BotManager(config);

manager.on('log', (msg) => console.log(`[Bot] ${msg}`));

manager.on('retry', (count) => {
  botState.reconnectAttempts = count;
});

manager.on('active', (newBot, username, meta) => {
  bot = newBot;
  botState.connected = true;
  botState.lastActivity = Date.now();
  botState.reconnectAttempts = 0;
  botState.activeBot = username;

  console.log(`[Bot] [+] ${username} is now active (${meta.role})`);
  if (config.discord && config.discord.events.connect) {
    sendDiscordWebhook(`[+] **${username} connected** to \`${config.server.ip}\``, 0x4ade80); // Green
  }

  currentIntervals = createIntervalManager();

  const mcData = require('minecraft-data')(config.server.version);
  const defaultMove = new Movements(newBot, mcData);
  defaultMove.allowFreeMotion = false;
  defaultMove.canDig = false;
  defaultMove.liquidCost = 1000;
  defaultMove.fallDamageCost = 1000;

  initializeModules(newBot, mcData, defaultMove, currentIntervals.add);
  setupLeaveRejoin(newBot);

  setTimeout(() => {
    if (bot === newBot && botState.connected) {
      newBot.chat('/gamerule sendCommandFeedback false');
    }
  }, 3000);

  setTimeout(() => {
    if (bot === newBot && botState.connected) {
      newBot.chat('/gamemode creative');
...
manager.on('inactive', (oldBot, username, meta) => {
  if (bot === oldBot) {
    botState.connected = false;
  }
  currentIntervals.clearAll();

  if (!meta.graceful && config.discord && config.discord.events.disconnect) {
    sendDiscordWebhook(`[-] **${username} disconnected** (${meta.reason || 'unknown'})`, 0xf87171); // Red
  }
});
...
// ============================================================
function initializeModules(bot, mcData, defaultMove, addInterval) {
  ...
  if (config.movement['circle-walk'].enabled) {
    startCircleWalk(bot, defaultMove, addInterval);
  }
  if (config.movement['random-jump'].enabled) {
    startRandomJump(bot, addInterval);
  }
  if (config.movement['look-around'].enabled) {
    startLookAround(bot, addInterval);
  }

  // ---------- CUSTOM MODULES ----------
  if (config.modules.avoidMobs) avoidMobs(bot, addInterval);
  if (config.modules.combat) combatModule(bot, mcData, addInterval);
  if (config.modules.beds) bedModule(bot, mcData, addInterval);
...
// MOVEMENT HELPERS
// ============================================================
function startCircleWalk(bot, defaultMove, addInterval) {
...
function startRandomJump(bot, addInterval) {
  addInterval(() => {
    if (!bot || !botState.connected) return;
    try {
      bot.setControlState('jump', true);
...
function startLookAround(bot, addInterval) {
  addInterval(() => {
    if (!bot || !botState.connected) return;
    try {
...
// Avoid mobs/players
function avoidMobs(bot, addInterval) {
...
// Combat module
function combatModule(bot, mcData, addInterval) {
  addInterval(() => {
    if (!bot || !botState.connected) return;
    try {
...
function bedModule(bot, mcData, addInterval) {
  addInterval(async () => {
    if (!bot || !botState.connected) return;
...
rl.on('line', (line) => {
  ...
  if (trimmed.startsWith('say ')) {
    ...
    console.log(`Connected: ${botState.connected}, Active: ${stats.activeBot}, Pool: ${stats.poolSize} bots, Uptime: ${formatUptime(Math.floor((Date.now() - botState.startTime) / 1000))}`);
  } else if (trimmed === 'reconnect') {
    console.log('[Console] Manual reconnect requested — ending the active bot (a replacement will connect automatically)');
    bot.end();
  } else if (trimmed === 'rotate') {
    console.log('[Console] Manual rotation requested — connecting a replacement bot now');
    if (!manager.forceHandover()) {
      console.log('[Console] Rotation already in progress, try again shortly');
    }
  } else {
    bot.chat(trimmed);
  }
...
process.on('uncaughtException', (err) => {
  ...
  // We just clear intervals and make sure a bot is (or gets) active again.
...
console.log(`Bot Pool: ${manager.accounts.length} accounts, rotating every ${manager.rotationMs / 3600000}h`);
console.log('='.repeat(50));
